...
// clang-format off
#include FUXI_EXTERNAL(absl/base/attributes.h) // ABSL_ATTRIBUTE_ALWAYS_INLINE
// clang-format on
...
template <typename C, typename T>
inline FUXI_CONSTEXPR int32_t BasicStringView<C, T>::compare(
...
inline constexpr BasicStringView<C, T>::operator ::std::initializer_list<
  ...
  return ::next::fuxi::InitializerList<value_type>(_data, _size);
}
...
template <typename C, typename T>
inline FUXI_CONSTEXPR bool operator<(
    typename ::std::common_type<BasicStringView<C, T>>::type left,
    BasicStringView<C, T> right) noexcept {
...
template <typename C, typename T>
inline FUXI_CONSTEXPR bool operator<(BasicStringView<C, T> left,
                                     BasicStringView<C, T> right) noexcept {
  return left.compare(right) < 0;
}
...
namespace std {
template <>
struct hash<::next::fuxi::StringView> {
  // absl和std c++20使用is_transparent定义来判定是否支持异构键
  using is_transparent = void;
  inline ABSL_ATTRIBUTE_ALWAYS_INLINE size_t
  operator()(::next::fuxi::StringView sv) const noexcept {
#if __GLIBCXX__
    return ::std::_Hash_impl::hash(sv.data(), sv.size());
#else
...
#if __GLIBCXX__
template <>
struct __is_fast_hash<hash<::next::fuxi::StringView>> : ::std::false_type {};
#endif

template <>
struct equal_to<::next::fuxi::StringView> {
  // absl和std c++20使用is_transparent定义来判定是否支持异构键
  using is_transparent = void;
  inline ABSL_ATTRIBUTE_ALWAYS_INLINE bool operator()(
      ::next::fuxi::StringView left,
      ::next::fuxi::StringView right) const noexcept {
    return left == right;
  }
};
} // namespace std
