...
// #include "fuxi/base/new.h" // operator new

// clang-format off
//#include FUXI_EXTERNAL(absl/time/clock.h) // absl::GetCurrentTimeNanos
...
ABSL_ATTRIBUTE_NOINLINE void ConcurrentBoundedQueue<T, S>::SlotFutex::
  ...
  int64_t begin_time_ns =
      timeout != nullptr ? Time::MonoTime().ToNanosecond(): 0;
  ...
  while (true) {
    ...
    // 必要的话，刷新等待时间
    if (timeout != nullptr) {
      int64_t end_time_ns = Time::MonoTime().ToNanosecond();
      int64_t wait_duration = Time::DurationFromTimespec(*timeout);
 
      wait_duration = wait_duration - (end_time_ns - begin_time_ns);
      if (wait_duration <= (0L)) {
        break;
      }
      modified_timeout = Time::ToTimespec(wait_duration);
...
ConcurrentBoundedQueue<T, S>::SlotFutex::spin_until_reach_expected_version_slow(
  ...
  int64_t end_time_ns =
      begin_time_ns +
      (timeout != nullptr
           ? Time::DurationFromTimespec(*timeout)
           : 0);
  while (true) {
    // 执行等待
    S::usleep(1000);
    ...
    // 检测是否超时
    if (timeout != nullptr) {
      int64_t time_ns = Time::MonoTime().ToNanosecond();
...
template <typename T, typename S>
ConcurrentBoundedQueue<T, S>::SlotVector::SlotVector(void* slot_buffer, size_t size) noexcept
    : _slots {reinterpret_cast<Slot*>(slot_buffer)},
      _size {size} {
...
template <typename T, typename S>
void ConcurrentBoundedQueue<T, S>::SlotVector::resize(void *slot_buffer, size_t size)  noexcept {
  this->~SlotVector();
  new (this) SlotVector(slot_buffer, size);
}
...
template <typename T, typename S>
/*static*/ size_t ConcurrentBoundedQueue<T, S>::reserve_buffer_size(size_t min_capacity) {
  auto new_capacity = ::absl::bit_ceil(min_capacity);
  // sizeof(Slot) * size, ::std::align_val_t(alignof(Slot))
  return sizeof(Slot) * new_capacity;
}

template <typename T, typename S>
size_t ConcurrentBoundedQueue<T, S>::reserve_and_clear(void *slot_buffer, size_t min_capacity) {
  auto new_capacity = ::absl::bit_ceil(min_capacity);
  bool create = !_slots.use_outer_buffer();
  if (create || (new_capacity != capacity())) {
    _slot_bits = __builtin_ctzll(new_capacity);
    _slot_mask = new_capacity - 1;
    _slots.resize(slot_buffer, new_capacity);
    _next_push_index.store(0, ::std::memory_order_relaxed);
    _next_pop_index.store(0, ::std::memory_order_relaxed);
    for (size_t i = 0; i < new_capacity; ++i) {
      _slots.futex(i).reset();
