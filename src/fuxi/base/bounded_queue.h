...
#include "fuxi/base/sched_interface.h"  // Futex
#include "fuxi/base/type_traits.h"      // IsInvocable
...
class ConcurrentBoundedQueue {
 private:
  ...
  struct alignas(FUXI_CACHELINE_SIZE) Slot {
    T value;
    SlotFutex futex;
  };
 ...
 public:
  ...
  size_t reserve_and_clear(void* slot_buffer, size_t min_capacity);
  static size_t reserve_buffer_size(size_t min_capacity);
 ...
 private:
  class SlotVector {
   public:
    ...
    SlotVector(void* slot_buffer, size_t size) noexcept;

    void resize(size_t size) noexcept;
    void resize(void* slot_buffer, size_t size) noexcept;
    ...
    bool use_outer_buffer() const { return use_outer_buffer_; }
  ...
  alignas(FUXI_CACHELINE_SIZE)::std::atomic<size_t> _next_push_index =
      ATOMIC_VAR_INIT(0);
  alignas(FUXI_CACHELINE_SIZE)::std::atomic<size_t> _next_pop_index =
      ATOMIC_VAR_INIT(0);
#else   // __clang__ || FUXI_GCC_VERSION >= 50000
  alignas(FUXI_CACHELINE_SIZE)::std::atomic<size_t> _next_push_index{0};
  alignas(FUXI_CACHELINE_SIZE)::std::atomic<size_t> _next_pop_index{0};
#endif  // __clang__ || FUXI_GCC_VERSION >= 50000
};

FUXI_NAMESPACE_END

#include "fuxi/base/bounded_queue.hpp"
