...
// #include "fuxi/base/absl_base_internal_invoke.h" //
// ::absl::base_internal::is_invocable_r
#include "fuxi/base/environment.h"
#include "fuxi/base/string_view.h"  // StringView
...
template <typename T>
struct TypeId {
#if !__clang__ && FUXI_GCC_VERSION < 90300
...
#define FUXI_HAS_STATIC_MEMBER_CHECKER(M, C)                \
  template <typename __FUXI_TPL_T>                          \
  struct C {                                                \
    template <typename __FUXI_TPL_U>                        \
    static auto checker(int) -> decltype(__FUXI_TPL_U::M);  \
    template <typename __FUXI_TPL_U>                        \
    static ::next::fuxi::NeverUsed checker(...);             \
    static constexpr bool value =                           \
        !::std::is_same<decltype(checker<__FUXI_TPL_T>(0)), \
                        ::next::fuxi::NeverUsed>::value;     \
  };
...
// 用法
// 1、创建检测器
// FUXI_DECLARE_STATIC_INVOCABLE(some_function, Checker);
// 2、使用检测器检测
// struct SomeClass {
...
#define FUXI_DECLARE_STATIC_INVOCABLE(F, C)                                 \
  template <typename __FUXI_TPL_T, typename... __FUXI_TPL_Args>             \
  struct C {                                                                \
    template <typename __FUXI_TPL_U>                                        \
    static auto checker(int)                                                \
        -> decltype(__FUXI_TPL_U::F(::std::declval<__FUXI_TPL_Args>()...)); \
    template <typename __FUXI_TPL_U>                                        \
    static ::next::fuxi::NeverUsed checker(...);                             \
    static constexpr bool value =                                           \
        !::std::is_same<decltype(checker<__FUXI_TPL_T>(0)),                 \
...
// 用法
// 1、创建检测器
// FUXI_DECLARE_MEMBER_INVOCABLE(some_function, Checker);
// 2、使用检测器检测
// struct SomeClass {
...
// Checker<SomeClass, Arg1, Arg2>::value == true
// Checker<int, Arg1, Arg2>::value == false
#define FUXI_DECLARE_MEMBER_INVOCABLE(F, C)                                \
  template <typename __FUXI_TPL_T, typename... __FUXI_TPL_Args>            \
  struct C {                                                               \
    template <typename __FUXI_TPL_U>                                       \
    static auto checker(int) -> decltype(::std::declval<__FUXI_TPL_U>().F( \
        ::std::declval<__FUXI_TPL_Args>()...));                            \
    template <typename __FUXI_TPL_U>                                       \
    static ::next::fuxi::NeverUsed checker(...);                            \
    static constexpr bool value =                                          \
        !::std::is_same<decltype(checker<__FUXI_TPL_T>(0)),                \
...
#define FUXI_MEMBER_INVOCABLE_CHECKER(F, C) FUXI_DECLARE_MEMBER_INVOCABLE(F, C)

// 检测是否可以进行hash计算
// 例如，如下检测可以通过
// IsHashable<std::hash<::std::string>, const std::string&>::value == true
// IsHashable<std::hash<::std::string>, const char*>::value == true
FUXI_DECLARE_MEMBER_INVOCABLE(operator(), IsHashable);
...
#define FUXI_TYPE_DEFINED_CHECKER(D, C)                                        \
  struct C {                                                                   \
    template <typename __FUXI_TPL_T>                                           \
    static typename __FUXI_TPL_T::D* checker(int);                             \
    template <typename __FUXI_TPL_T>                                           \
    static void checker(...);                                                  \
    template <typename __FUXI_TPL_T>                                           \
    static constexpr bool value() {                                            \
      return !::std::is_same<void, decltype(checker<__FUXI_TPL_T>(0))>::value; \
    }                                                                          \
    template <typename __FUXI_TPL_T>                                           \
    using type = typename ::std::remove_pointer<decltype(                      \
        checker<__FUXI_TPL_T>(0))>::type;                                      \
  };
...
#include "fuxi/base/type_traits.hpp"
