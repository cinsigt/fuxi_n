...
#ifndef __CUDACC__
#define FUXI_CONSTEXPR constexpr
#else
#define FUXI_CONSTEXPR
#endif
...
class BasicStringView {
 public:
  ...
  inline FUXI_CONSTEXPR int32_t compare(size_type position, size_type count,
...
#if !__GLIBCXX__
  // LIBCPP环境下std::string_view和::next::fuxi::BasicStringView是无关类型
  // 这里主动增加转换功能，保持可以无缝混用
...
namespace std {
template <typename C, typename T = std::char_traits<C>>
class basic_string_view : public ::next::fuxi::BasicStringView<C, T> {
 public:
...
#include "fuxi/base/string_view.hpp"
